import {
  ...
  BreakStmt, ContinueStmt, ValueStmt, UseStmt, TraitStmt, VocabularyStmt,
...
export class AstPrinter implements ExprVisitor<string>, StmtVisitor<string>, TypeVisitor<string> {
  ...
  visitFunctionStmt(stmt: FunctionStmt): string {
    ...
    let s = `(${stmt.isOperator ? "operator " : ""}${stmt.isMutating ? "var-fun" : "fun"} ${stmt.name.lexeme}`;
  ...
  visitVocabularyStmt(stmt: VocabularyStmt): string {
    const members = stmt.members.map(m => m.lexeme).join(" ");
